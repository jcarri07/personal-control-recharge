...
$sql = "SELECT * FROM formacion_actual WHERE id_usuario = '$idUser' AND estatus = 'activo';";
$query = mysqli_query($conn, $sql);
...
if ($datos_personales['step'] > 3) {
                                ...
                                <div class="page-header-breadcrumb">
                                    <ul class="breadcrumb-title">
                                        ...
                                        <li class="breadcrumb-item active">
                                            <a class="activate">Actualización de Datos / Formación Actual</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                                ...
                                                <div id="wizard">
                                                    <section>
                                                        ...
                                                        if (mysqli_num_rows($query) > 0) {
                                                                ...
                                                                <table id="row-select" class="table table-striped table-bordered nowrap">
                                                                    <thead>
                                                                        <tr>
                                                                            ...
                                                                            <th>Año Estimado de Graduación</th>
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        while ($row = mysqli_fetch_array($query)) {
                                                                            $id_formacion_actual = $row['id_formacion_actual'];
                                                                            $estudios_en_curso = $row['estudios_en_curso'];
                                                                            $anio_estimado_graduacion = $row['anio_estimado_graduacion'];
                                                                            $instituto_universidad = $row['instituto_universidad'];
                                                                            $observaciones = $row['observaciones'];
                                                                            ?>
                                                                            <tr>
                                                                                <td><?php echo ++$i; ?></td>
                                                                                <td><?php echo htmlspecialchars($estudios_en_curso); ?></td>
                                                                                <td><?php echo htmlspecialchars($anio_estimado_graduacion); ?></td>
                                                                                ...
                                                                                <td>
                                                                                    <button class="btn btn-primary btn-table"
                                                                                        onclick="openModalEdit('<?php echo $id_formacion_actual; ?>', '<?php echo addslashes(htmlspecialchars($estudios_en_curso)); ?>', '<?php echo addslashes(htmlspecialchars($anio_estimado_graduacion)); ?>', '<?php echo addslashes(htmlspecialchars($instituto_universidad)); ?>', '<?php echo addslashes(htmlspecialchars($observaciones)); ?>');">
                                                                                        <i class="feather icon-edit"></i>
                                                                                    </button>
                                                                                    <button class="btn btn-danger btn-table"
                                                                                        onclick="deseaEliminar('<?php echo $id_formacion_actual; ?>');">
                                                            ...
                                                            ?>
                                                            <h2 class="text-center mt-4">No hay registros de formación actual</h2>
                                                            <?php
                                                        }
                                                        ?>
                                                    </section>
                        ...
                        <div class="auth-box card-block">
                            <div class="row m-b-0">
                                <div class="col-md-12 text-center" style="margin-bottom: 0px;">
                                    <div class="box-title">
                                        <h3 class="modal-title text-center" style="margin-top: 0px; padding-top: 5px; padding-bottom: 5px;">Formación Actual</h3>
                                    </div>
                                    <div class="form-group row justify-content-center">
                                        <div class="col-md-6">
                                            <label class="block text-left" style="text-align: left; display: block;">Estudios en Curso</label>
                                            <input name="estudios_en_curso" id="estudios_en_curso" type="text" class="form-control" maxlength="200" required>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="block text-left" style="text-align: left; display: block;">Año Estimado de Graduación</label>
                                            <input name="anio_estimado_graduacion" id="anio_estimado_graduacion" type="text" class="form-control" maxlength="10" required>
                                        </div>
    ...
    <script>
        ...
        function openModalAdd() {
            ...
            $("#aux_estatus").text("");
            $("#form-edit")[0].reset();
            $("#modal-form .modal-title").text("Añadir Formación Actual");
            $("#modal-form form button[type=submit]").text("Añadir");
            $("#modal-form").modal("show");
        }

        function openModalEdit(id, estudios_en_curso, anio_estimado_graduacion, instituto_universidad, observaciones) {
            $("#aux_id").text(id);
            $("#aux_opc").text("edit");
            $("#aux_estatus").text("");

            $("#estudios_en_curso").val(estudios_en_curso);
            $("#anio_estimado_graduacion").val(anio_estimado_graduacion);
            $("#instituto_universidad").val(instituto_universidad);
            $("#observaciones").val(observaciones);

            $("#modal-form .modal-title").text("Editar Formación Actual");
        ...
        $("#form-edit").on("submit", function (event) {
            ...
            datos.append('id_formacion_actual', $("#aux_id").text());
            datos.append('opc', $("#aux_opc").text());

            datos.append('estudios_en_curso', this.estudios_en_curso.value);
            datos.append('anio_estimado_graduacion', this.anio_estimado_graduacion.value);
            ...
            $.ajax({
                url: '../modules/update-data-user/formacion-actual-process.php',
                type: 'POST',
                data: datos,
                cache: false,
                contentType: false,
